...
#binwith groups the data into chunks, you can change the scale
```
...
mean_amp <- data_after_exclusions %>% 
  summarise(mean_amp = mean(amp_score)) %>% 
  pull(mean_amp)


ggplot(data = data_after_exclusions,
       aes(x = amp_score)) +
  geom_histogram(binwidth = 0.1,boundary =0.1, colour = "black")+
  scale_x_continuous( breaks = seq(0,1,0.1),
                      name = "AMP Score",
                      limits = c(0, 1)) +
  scale_y_continuous(breaks = seq(0,60,10),
                     name = "Frequency")+
  geom_vline(xintercept = mean_amp, linetype = "dotted", colour = "red") +
...
ggplot(data = data_after_exclusions,
  ...
  geom_col(fill="pink",
           colour = "violet",
           width = 0.6)+
   geom_errorbar(aes(xmin = mean_self_report - amp_se, 
                    xmax = amp_mean + amp_se), 
                width = 0.1, 
                color = "black") +
  labs(title = "Bar Plot of with Standard Errors",
       x = "GeMean AMP scorender",
       y = "Gender") +
  coord_flip()+
  theme_linedraw() 
