...
```{r}
test <- data_demographics_raw %>% 
  count(subject)

test2 <- data_selfreport_raw %>% 
  count(subject)

test3 <- data_amp_raw %>% 
  count(subject)

# There are different numbers in each data frames because its always the same participants, but in each dataset the number of answers of each subject is different. Shortly said it differs in the amount of rows for each participant (a row for each trial).

```

- Why are the numbers not round?
 It is because not every participant has the same amount of trials 
...
How would you bring up the help menu to understand how `janitor::clean_names()` works?
  I would write ?clean_names in the console  
...
data_selfreport_trimmed <- read_csv(file = "../data/raw/data_selfreport_raw.csv") %>% 
  clean_names() %>% 
  rename(unique_id = subject,
         item = trialcode) %>%
  select(unique_id,item,response)

data_amp_trimmed <- read_csv(file = "../data/raw/data_amp_raw.csv") %>% 
  clean_names() %>% 
  rename(unique_id = subject,
         block = blockcode,
         trial_type = trialcode) %>% 
  select(unique_id,block,trial_type,latency)
...
# this are not columns so it cannot be counted
 # data_amp_trimmed %>%
 #   count(correct)
# 
# data_amp_trimmed %>%
...
# Step 1: Calculate mean scores and count of items
mean_scores <- data_selfreport_tidy %>%
  group_by(unique_id) %>%
  summarise(
    mean_score = mean(c(like, positive, prefer), na.rm = TRUE),
    count_items = sum(!is.na(c(like, positive, prefer))),
    .groups = 'drop'
  ) %>%
  mutate(complete_data = count_items == 3)

# Step 2: Filter participants with complete data
complete_data <- mean_scores %>%
  filter(complete_data)

# Step 3: Calculate the mean and standard deviation of the mean scores for participants with complete data
mean_of_means <- mean(complete_data$mean_score)
sd_of_means <- sd(complete_data$mean_score)

# Display results
print(paste("Mean of participants' mean scores with complete data:", mean_of_means))
print(paste("Standard deviation of participants' mean scores with complete data:", sd_of_means))
...
# Step 2: Calculate the proportion of participants to be excluded
exclusion_rate <- sum(amp_data$exclude_amp == "exclude") / nrow(amp_data)

# Display the resulting data frame and the exclusion rate
print(amp_data)
print(paste("Proportion of participants to be excluded:", exclusion_rate))
