...
# are there duplicates -> sanity check?
...
data_amp_performance_criteria <- data_amp_tidied_complete %>% 
  ...
  # flag unique ID and write exclude include in a new row
...
-count the number of uniue_id with correct/wrong number of response
-exclude them
-check your exclusion worked
...
data_selfreport_trial_tidy <- data_selfreport_raw %>% 
  rename(unique_id = subject,
         item_type = trialcode) %>% 
  select(unique_id,response,item_type) %>% 
