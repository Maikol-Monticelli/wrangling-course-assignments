...
data_selfreport_tidied %>%
    ...
    count(n_trials, name = "n_participants") %>% 
    arrange(desc(n_participants))
# You can do the same by grouping the data by unique_id and summarize it and afterward group it again and summarize it again

  
# Cleaning out wrong number of trials

data_complete_trials <- data_selfreport_tidied %>% 
  count(unique_id, name = "n_trials")  %>% 
  filter(n_trials == 3) 

#Joining the Dataset togheter

data_complete_trials_complete <- data_selfreport_tidied |>
  semi_join(data_complete_trials, by = "unique_id")

#you can do the same this way, without doing the joining afterward joining 
...
data_selfreport_trial_level <- data_complete_trials_complete %>% 
