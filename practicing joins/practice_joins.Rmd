...
data_combined <- full_join(data_age_gender_subset,
            data_amp_summary_subset,
            by = "unique_id")

# To join it completely you use full_join
...
data_self_reports_and_their_amp_data <- left_join(data_selfreport_summary_subset,
                                                  data_amp_summary_subset,
                                                  by = "unique_id")
...
data_amp_data_and_their_self_reports <- right_join(data_selfreport_summary_subset,
                                                  data_amp_summary_subset,
                                                  by = "unique_id")
...
data_combined_2 <- inner_join(data_selfreport_summary_subset,
                                                  data_amp_summary_subset,
                                                  by = "unique_id")
...
create 'data_missing_ids' which should list the unique_ids that are missing from data_unique_id_subset but are present in at least one of data_age_gender_subset, data_amp_summary_subset, and data_selfreport_summary_subset. This will require two different joins. Which? Implement them.

```{r}
data_age_amp_selfreport <- full_join(data_age_gender_subset,
                           data_amp_summary_subset,
                           data_selfreport_summary_subset,
                           by = "unique_id")
data_missing_ids <- anti_join(data_age_amp_selfreport,
                              data_unique_id_subset,
                               by = "unique_id")

# Problem of full-join is, that it retains a lot of dublicates, cleaning them out is sometimes a struggle, especially finding out wich are the correct values if they change
